...
const ProjectItem = ({
  ...
  return (
    ...
    >
      {/* ✅ 썸네일 영역: video 유무 상관없이 클릭하면 상세 모달 */}
      <button
        type='button'
        onClick={handleThumbnailClick}
        className='relative aspect-video rounded-lg overflow-hidden cursor-pointer text-left'
      >
        {project.video ? (
          <video
            src={project.video}
      ...
      <div>
        ...
        <div className='flex gap-4'>
          ...
          {/* ✅ video가 있는 프로젝트만 Video 버튼 노출 */}
          {project.video && (
            <button
              type='button'
              onClick={() => onVideoClick(project.video as string, index)}
              className='text-sm text-[#40F8D2] hover:underline inline-flex items-center'
            >
              Video <ArrowRight className='ml-1 h-4 w-4' />
            </button>
          )}
